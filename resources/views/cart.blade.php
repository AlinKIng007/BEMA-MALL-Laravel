    ...
    @foreach ($results as $result)
        <div class="cart-item flex items-center justify-between p-4 border-b">
            <div class="flex items-center space-x-4">
                <img src="images/{{ $result->image_1 }}" alt="{{ $result->product_name }}" class="w-20 h-20 object-cover rounded-lg">
                <div>
                    <h3 class="font-semibold text-gray-800">{{ $result->product_name }}</h3>
                    <p class="text-sm text-gray-600">{{ $result->description }}</p>
                    <p class="text-orange-600 font-semibold">${{ number_format($result->price, 2) }}</p>
</div>
    ...
    <div></div>
            ...
            <div class="flex items-center space-x-4">
                <div class="flex items-center space-x-2">
                    <span id="cart-amount:{{ $result->id }}" class="w-8 text-center">{{ $result->amount }}</span>
                </div>
...
@endif
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <span id="subtotal" class="text-xl font-bold text-orange-600">${{ $subtotal }}</span>
            </div>
            <div class="flex justify-between items-center mb-4">
                <span class="text-lg font-semibold text-gray-700">Shipping:</span>
                <span class="text-xl font-bold text-orange-600">${{ $amount *  2.5}}</span>
            </div>
            <div class="flex justify-between items-center mb-6">
                <span class="text-lg font-semibold text-gray-700">Total:</span>
                <span id="total" class="text-2xl font-bold text-orange-600">${{ $subtotal +($amount*2.5) }}</span>
            </div>
            @if ($amount > 0)
