                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="transition-colors duration-200 ease-in-out order-row">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $order->product_name ? $order->product_name : 'N/A' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $order->amount }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold" style="color: #ea580c;">${{ number_format($order->price, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold" style="color: #ea580c;">${{ $order->price * $order->amount }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                style="padding:10px;
                                @if($order->level == 1) background-color: #dbeafe; color: #1e40af;
                                        @elseif($order->level == 2) background-color: #dcfce7; color: #15803d;
                                        @elseif($order->level == 3) background-color: #fee2e2; color: #b91c1c;
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $order->method_name ? $order->method_name : 'N/A' }}</td>
<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
    {{ $order->time_of_purchase ?? 'N/A' }}
</td>
