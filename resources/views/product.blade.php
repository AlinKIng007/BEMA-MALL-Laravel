...
<div class="product-container">
    <div class="product-card">
        <div class="product-image">
            <img src="{{ asset('images/' . $products->image_1) }}"
                 alt="{{ $products->product_name }}"
                 class="main-image"
                 data-product-image="{{ asset('images/' . $products->image_1) }}">
        </div>
        <div class="product-details" data-product-id="{{ $products->id }}" data-max-quantity="{{ $products->amount }}">
            <h1 class="product-title" data-product-name="{{ $products->product_name }}">
                {{ $products->product_name }}
            </h1>
            <div  class="price-tag" data-product-price="{{ $products->price }}">
                ${{ $products->price }}
            </div>
            ...
            <div class="product-meta">
                <div id="div-item-stock" class="meta-item stock">
                    ...
                    <span id="item-stock">In Stock: {{ $products->amount }}</span>
                </div>

                <div class="meta-item mall">
                    <i class="fas fa-store"></i>
                    <span>Mall: {{ $products->mall_name }}</span>
        ...
        @foreach ($grouped_variations as $variation_name => $values)
            @if ($variation_name === 'color')
                ...
                <div class="color-options">
                    <label>{{ ucfirst($variation_name) }}:</label>
                    <div class="color-selector">
                        @foreach ($values as $value)
                            <button class="color-btn" id="vo_id:{{ $value->vo_id }}" style="background-color: {{ $value->variation_value }};" data-value="{{ $value->variation_value }}">
                            </button>
                        @endforeach
                    </div>
                </div>
            @else
                <style>
                    .{{ $variation_name }}-options {
                        margin: 1.5rem 0;
                    }

                    .{{ $variation_name }}-selector {
                        display: flex;
                        gap: 1rem;
                        margin-top: 0.5rem;
                    }
                    ...
                    .{{ $variation_name }}-btn:hover {
                        background-color: #f0f0f0;
                    }

                    .{{ $variation_name }}-btn.active {
                        background-color: #dcdcdc;
                        border: 2px solid #2d3436;
                    }
                </style>

                <div class="{{ $variation_name }}-options">
                    <label>{{ ucfirst($variation_name) }}:</label>
                    <div class="{{ $variation_name }}-selector">
                        @foreach ($values as $value)
                            <button class="{{ $variation_name }}-btn" id="btn-id:{{ $value->vo_id }}" data-value="{{ $value->variation_value }}">
                                {{ strtoupper($value->variation_value) }}
                            </button>
                        @endforeach
                    </div>
                </div>
...
<form id="addToCartForm" action="/cartadd" method="POST">
            ...
            <div class="quantity-control">
                ...
                <div class="quantity-wrapper">
                    ...
                    <input type="number" id="quantity" name="amount" value="1" min="1" max="{{ $products->amount }}">
            ...
            <div class="action-buttons">
                <button class="btn wishlist-btn" id="wishlist-button" onclick="addToWishlist()">
                    ...
                    <input type="text" id="p_id" name="product_id" style="display: none">


                    <input type="text" id="user_id" value="{{ Auth::user()->id }}" name="user_id" style="display: none">
...
<script>
    ...
    document.getElementById("item-stock2").value = newValue;
